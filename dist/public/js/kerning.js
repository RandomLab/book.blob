        ...
        function injector(t, splitter, klass, after) {
            if (klass.localeCompare("word") == 0){
                ...
                for(var i = 0 ; i < a.length ; i++){
                    //console.log(a[i]);
                    var as = $.parseHTML(a[i]);
                    //console.log(as);
                    for (var j = 0 ; j < as.length ; j++){
                        if(as[j].nodeName.localeCompare("EM")==0){
                            inject += '<em class="'+klass+(i+1)+'">'+as[j].innerText+'</em>'+after;
                        }
                        if(as[j].nodeName.localeCompare("#text")==0){
                            inject += '<span class="'+klass+(i+1)+'">'+as[j].textContent+'</span>'+after;
                        }
                        if(as[j].nodeName.localeCompare("STRONG")==0){
                          inject += '<strong class="'+klass+(i+1)+'">'+as[j].innerText+'</strong>'+after;
                        }
                ...
                for(var property in css[selector]) {
                    ...
                    if(match = property.match(new RegExp('^(-' + browserPrefix + '|-' + osPrefix +')?-(letter|word)-(kern|transform|size|color|backgroundcolor|weight)', 'i'))) {
                        ...
                        elements
                            ...
                            .lettering('words').children('*').css('display', 'inline-block') // break down into words
